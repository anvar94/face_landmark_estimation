...
def eye_aspect_ratio(preds, pts):
    N = preds.size(0)
    L = preds.size(1)//2
    ...
    for i in range(N):
        ...
        right_eye_pts = lmk_pts[42:48]
        left_eye_pts = lmk_pts[36:42]

        lips_aspect_pts = lmk_pts[60:68]

        right_eye_pred = lmk_pred[42:48]
        left_eye_pred = lmk_pred[36:42]
        lips_aspect_pred = lmk_pred[60: 68]



        aspect_right_eye_pts = eye_aspect_rationation(right_eye_pts)
        aspect_left_eye_pts = eye_aspect_rationation(left_eye_pts)
        aspect_right_eye_pred = eye_aspect_rationation(right_eye_pred)
        aspect_left_eye_pred = eye_aspect_rationation(left_eye_pred)

        lips_aspect_ratio_pts = eye_aspect_rationation(lips_aspect_pts)
        lips_aspect_ratio_pred = eye_aspect_rationation(lips_aspect_pred)

        right_diff_eye = aspect_right_eye_pts - aspect_right_eye_pred
        left_diff_eye = aspect_left_eye_pts - aspect_left_eye_pred

        return right_diff_eye, left_diff_eye, aspect_right_eye_pts, aspect_left_eye_pts, aspect_right_eye_pred, aspect_left_eye_pred, lips_aspect_ratio_pts, lips_aspect_ratio_pred
...
def Init(args):
    ...
    with open(os.path.join('/home/hclee/Desktop/ATF-master/experiments','{}.json'.format(args.data_type))) as f:
        cfg = json.load(f)
        cfg["LR"] = args.lr
        cfg["GPUS"] = args.gpus
